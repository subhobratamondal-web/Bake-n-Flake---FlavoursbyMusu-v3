...
const mapIframeRep = `{msg.mapIframe && (
                          <div className="mt-6 rounded-2xl overflow-hidden border border-slate-200 dark:border-white/10 shadow-lg aspect-video h-48 md:h-56 w-full max-w-full relative z-10 isolate" style={{ pointerEvents: 'auto' }}>
                             <iframe src={msg.mapIframe} className="w-full h-full border-0" allowFullScreen loading="lazy" referrerPolicy="no-referrer-when-downgrade"></iframe>
                          </div>
                        )}`;
